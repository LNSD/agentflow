...
This document defines the three-tier testing strategy used in this project: **unit**, **integration**, and **e2e** tests. The unit and integration tiers each have two variants — **in-tree** (inside `src/`) and **public API** (in `<crate>/tests/`) — distinguished by the `it_*` naming convention.
