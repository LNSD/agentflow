# Technical Overview for Coding Agents
...
1. **Use Skills for operations** → Invoke skills (`/code-format`, `/code-check`, `/code-test`, `/feature-discovery`, `/feature-fmt-check`, `/feature-validate`) instead of running commands directly
...
| "What features does this project have?", "What can this project do?" | `/feature-discovery` |
...
- Writing feature docs? → `/feature-fmt-check` + [feature-docs.md](docs/__meta__/feature-docs.md)
...
**USE Skills (`/code-format`, `/code-check`) for all common operations. Only use `cargo` or other tools directly when the operation is NOT covered by a skill or NOT available in the justfile.**
...
- ✅ **Direct tool OK**: `cargo run -p <binary> -- --help` (running binaries not in justfile)
